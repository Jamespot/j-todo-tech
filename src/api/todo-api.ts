...
class TodoApi {
  ...
  private buildErrorResponse = function buildErrorResponse(
    code: number,
    description: string
  ): ApiResponseError {
    return {
        ...
        if (this.isSuccess(this.successRate)) {
          try {
            if (
              ...
              todoIndex < 0 ||
              todoIndex >= this.todos[listIndex].items.length
            ) {
              reject(this.buildErrorResponse(400, "index out of bound"));
            }
        ...
        if (this.isSuccess(this.successRate)) {
          try {
            if (
              ...
              listIndex >= this.todos.length ||
              sourceIndex < 0 ||
              sourceIndex >= this.todos[listIndex].items.length
            ) {
              reject(this.buildErrorResponse(400, "index out of bound"));
            }
        ...
        if (this.isSuccess(this.successRate)) {
          try {
            if (
              ...
              listIndex >= this.todos.length ||
              itemIndex < 0 ||
              itemIndex >= this.todos[listIndex].items.length
            ) {
              reject(this.buildErrorResponse(400, "index out of bound"));
            }
